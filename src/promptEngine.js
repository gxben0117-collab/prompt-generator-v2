...
function faceMasterControlText() {
  return [
    "臉部主控：若有上傳角色圖片，以上傳照片中的臉部作為唯一身份來源；若沒有上傳圖片，請先建立一位 AI絕世美人 虛擬真人角色作為本次唯一身份來源",
    "有上傳圖時先鎖定原始真人臉；無上傳圖時固定 AI絕世美人 的臉型、眼型、鼻型、嘴型、成熟年齡感與真實皮膚質感",
    "髮型只能微調，不改有圖時的髮際線、臉型輪廓或真人身份；無圖時不得漂移成多張不同 AI 臉",
    "不可重畫、美化或換成古裝女主角臉；有圖時保留原始臉型、眼距、眼型、眼皮結構、鼻翼寬度、嘴唇形狀、下顎線、臉頰肉感、成熟年齡感、自然不對稱與皮膚質感；無圖時保留 AI絕世美人 固定五官",
    "臉部角度接近原圖；無圖時用正面或微側正面，雙眼清楚看向鏡頭",
...
function buildFinalIdentityText(form = DEFAULT_FORM) {
  return [
    "角色身份鎖定：有上傳角色圖片時，保留上傳照片原始臉型、眼型、鼻型、嘴型、下顎線、五官比例、成熟年齡感與真實皮膚紋理；沒有上傳圖片時，先建立一位 AI絕世美人 虛擬真人角色作為唯一主角，固定她的臉型、眼型、鼻型、嘴型、五官比例與皮膚質感。全程不換臉、不生成第二位新演員臉、不漂移成網紅臉；髮型與髮飾可配合角色微調，但不得改變臉型、髮際線與真人辨識度，也不得破壞無圖時的 AI絕世美人 固定身份。",
