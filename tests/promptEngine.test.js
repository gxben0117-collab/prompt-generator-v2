...
describe("prompt engine", () => {
  ...
  it("maps world templates into the correct parent role categories", () => {
    ...
    expect(parentCategoryForProfile(byId("ethnic-oldtown-red-hanfu-lantern-lady"))).toBe("民族古城旅拍");
    expect(parentCategoryForProfile(byId("ethnic-oldtown-silver-miao-festival-maiden"))).toBe("民族古城旅拍");
  ...
  it("ships worldview layer profiles for one-click costume systems", () => {
    ...
    expect(parentCategoryForProfile(WORLD_LAYER_PROFILES.find((profile) => profile.title === "小龍女・古墓仙影"))).toBe("歷史小說名著人物");
          ...
          : /敦煌|飛天|莫高窟|敦煌壁畫|鳴沙|月牙泉|洞窟/.test(profileText)
            ...
            : /nine-tail|九尾|狐仙|狐姬|狐后|妖狐|靈狐|天狐|青丘/.test(profileText)
              ...
              : /金庸|神鵰|小龍女|古墓派/.test(profileText)
                ? "歷史小說名著人物"
          : /succubus|魅魔|夜宴魅姬|冰霜夜宴魅姬|莉莉絲|高訂暗黑禮服/.test(profileText)
            ? "魅魔"
    ...
    expect(WORLD_LAYER_PROFILES.map((profile) => profile.title)).toContain("紅裳燈影・古城漢服仕女");
    expect(WORLD_LAYER_PROFILES.map((profile) => profile.title)).toContain("銀冠鼓樓・苗風節慶少女");
    ...
    expect(succubus.layers.costumeLayer2).toContain("禮服式柔性胸腰支撐");
    expect(succubus.layers.costumeLayer2).toContain("同一件長裙完整覆蓋");
    expect(succubus.costume).toContain("哥德式華麗配件");
    expect(succubus.layers.costumeLayer10).toContain("哥德式華麗配件");
    ...
    expect(frostQueen.layers.costumeLayer6).toContain("冷白絲絨禮服式外袍");
  ...
  it("ships formal succubus queen and fallen angel queen role cards", () => {
    const byId = (id) => WORLD_LAYER_PROFILES.find((profile) => profile.id === id);
    const formalSuccubi = WORLD_LAYER_PROFILES.filter((profile) => profile.id.startsWith("formal-succubus-queen-"));
    const formalFallen = WORLD_LAYER_PROFILES.filter((profile) => profile.id.startsWith("formal-fallen-queen-"));
    const nightgownSuccubus = byId("darkroyal-succubus-1");
    const nightgownFallen = byId("darkroyal-fallen-1");

    expect(formalSuccubi).toHaveLength(10);
    expect(formalFallen).toHaveLength(10);
    expect(nightgownSuccubus.title).toBeTruthy();
    expect(nightgownFallen.title).toBeTruthy();
    expect(formalSuccubi[0].costume).toContain("黑色蕾絲馬甲式胸衣");
    expect(formalSuccubi[0].costume).toContain("紅寶石鑲嵌");
    expect(formalFallen[0].costume).toContain("月銀色真絲高腰長裙");
    expect(formalFallen[0].costume).toContain("黑曜石色胸腰護甲");
    expect(parentCategoryForProfile(formalSuccubi[0])).toBe("魅魔");
  ...
  it("keeps night-banquet charm profiles soft, cinematic, and separate from black-wing epic templates", () => {
    ...
    for (const profile of profiles) {
      ...
      expect(text).toMatch(/禮服式|robe|絲絨/);
  ...
  it("supports vivid visual weight controls without changing the five-field output shape", () => {
    ...
    expect(darkBanquet).toContain("主體是 one-piece bias-cut silk evening gown");
  ...
  it("adds dark royal original-body safety only for dark fantasy categories", () => {
    ...
    expect(darkRoyal).toContain("魅魔夜宴連身緞面禮服裙");
    expect(darkRoyal).toContain("one-piece bias-cut silk evening gown");
    ...
    expect(inferredDarkRoyal).toContain("分類：夜宴魅魔／高訂暗黑禮服電影");
  ...
  it("keeps succubus styling as a one-piece evening gown instead of underwear", () => {
    const prompt = buildPrompt({
      theme: "紫蝶夜宴魅魔",
    ...
    expect(prompt).toContain("連身緞面禮服裙");
    expect(prompt).toContain("one-piece bias-cut silk evening gown");
  ...
  it("expands short costume text into a cinematic layer system", () => {
    const form = expandCostumeToLayers({
      theme: "紫蝶夜宴魅魔",
      costume: "紫色蝴蝶 蕾絲邊 暗黑禮服 哥德式配件 鍊條 透明紗",
    });
    const prompt = buildPrompt(form);

    expect(form.costumeLayer1).toContain("紫色低胸連身真絲緞面禮服裙");
