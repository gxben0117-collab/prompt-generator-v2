...
**報告 A（本頁第二節）：** 模組化改造方案（2026-06-13，基於 XXXXXX.txt 評審）  
**報告 B（下方歷史報告）：** 邏輯修正方案（2026-06-07，全專案掃描）

---

### 核心問題診斷對比

| 維度 | 報告 A（模組化方案） | 報告 B（邏輯修正方案） |
|------|---------------------|---------------------|
| **問題定位** | 古裝模板殘留 80%+，通用模板無法依主題切換 | 角色卡 sceneAction 含矛盾指令（持杯 vs 不持杯同時存在） |
| **症狀** | 巴黎咖啡館出現披帛/戰袍/dominant cinematic silhouette | 寢宮寵妃輸出「持酒盞」+「不預設拿酒杯」兩個矛盾句 |
| **根因** | 服裝/動作/光影元素未按主題分類 | `enrichSceneAction()` 在原始動作後追加補強，無法覆蓋舊動作 |
| **影響範圍** | 所有主題（古裝元素污染現代/都市/旅拍場景） | 350 張寢宮類角色卡 + 74 組持杯動作 + 16 組硬道具 |
...
```yaml
核心模板（永久保留，所有主題共用）:
  - 身份鎖定/真人骨架/構圖/妝容規則/安全姿態/負面詞

服裝模組（依主題切換）:
  皇后模組: layer4 披帛、layer6 戰袍、dominant cinematic silhouette
  旅拍模組: layer4 輕便外套、layer6 去除、natural travel outfit
  都市模組: layer4 風衣西裝、layer6 去除、modern urban fashion
  仙俠模組: layer4 仙氣披帛、layer6 flowing silk drapery
  祭司模組: layer4 神殿祭服、layer6 古典垂褶長裙

動作模組（依主題切換）:
  旅拍動作: 坐石階/扶欄回望/踏步抓拍/背包手鼓
  宮廷動作: 撫琴倚案/扶欄倚窗/持扇持燈/臨鏡梳妝
  神話動作: 踏蓮雲階/持法器托銀杯/扶石柱回身
  都市動作: 端咖啡翻書/倚窗凝視/坐吧台沙發

光影模組（依主題切換）:
  都市攝影: 窗光/街燈/咖啡館暖光 + Editorial Fashion Photography
  高端旅拍: 黃昏暖光/雪山冷光 + Cinematic Travel Photography
  宮廷金光: 宮燈/燭火/金箔反射 + 高亮商業古裝電影海報
```

**實作步驟：**
1. 更新 `doc/核心咒語規範.txt`：新增模組定義區段
2. 建立 `src/modules/` 目錄（core.js, costume.js, action.js, light.js, themeMapping.js）
3. 修改生成邏輯：`generatePrompt(roleCard, theme)` 依 THEME_MODULES 映射動態組裝
4. 同步至 `src/coreSpec.js`：`npm run sync:spec`
5. 更新測試：主題分類正確性、模組切換、不相容元素不混入

**優點：**
- ✅ 治本：從架構層面消除主題污染
- ✅ 擴展性強：新主題只需加映射，不需改核心模板
- ✅ 維護性好：模組化分離，職責清晰

**缺點：**
- ⚠️ 工程量大：需建立完整模組系統（5 個新檔案 + 邏輯重構）
- ⚠️ 向後相容風險：2,202 張現有角色卡需設計遷移方案
- ⚠️ 預設模組選擇：未分類主題需合理 fallback 策略
- ⚠️ 文檔同步：`doc/核心咒語規範.txt` 和 `src/coreSpec.js` 需持續一致
...
1. **清理 `actionQualityGuardText()` 流程文字（data.js 約行 7132）：**
   ```diff
   ...
   - ChatGPT 依...設計
   + （直接移除 "ChatGPT" 字眼，改用畫面導演語言）
   ```

2. **修改 `profileFactory.js` 第 75 行道具句：**
   ```diff
   - ${prop}作為角色記憶點，與手部動作自然互動
   + ${prop}可依場景作為手持、支撐點、桌面、床邊、前景或近景道具，不必固定拿在手上
   ```

3. **建立 `normalizeSceneActionProps(profile)`（在 enrichSceneAction 組裝前執行）：**
   ```js
   function normalizeSceneActionProps(profile) {
     if (!isBedchamberConsortProfile(profile)) return profile.sceneAction;
     
     let action = profile.sceneAction;
     // 寢宮類遇到持杯 → 改為場景陳設
     action = action.replace(/持酒杯|持酒盞|端盞|托盞|持杯/g, '酒盞置於床邊小几');
     action = action.replace(/提燈|持燭台|手持宮燈/g, '宮燈作床邊光源');
     action = action.replace(/持權杖|扶權杖/g, '權杖靠床榻側邊');
     
     return action;
   }
   ```

4. **強化 `isBedchamberConsortProfile()` 正則（data.js 約行 7105）：**
   ```diff
   - /寢宮|寵妃|床榻|臥榻|軟榻|王榻|睡衣寵妃|旗袍寵妃|魅魔寵妃|宮燈寵妃/
   + /寢宮|寵妃|床榻|臥榻|軟榻|王榻|睡衣寵妃|旗袍寵妃|魅魔寵妃|宮燈寵妃|睡衣|私房|臥室/
   ```

**P1 資料清理：**
- 人工清理 `handcraftedConsortProfiles.js` 原始 sceneAction（74 組持杯 + 16 組硬道具）
- 保留合理持物場景：吧台/宴會/茶席/戰場/祭儀

**優點：**
- ✅ 執行快：只改 3-4 個函數 + 資料清理
- ✅ 風險低：不破壞現有架構
- ✅ 立即見效：解決當前矛盾指令問題
- ✅ 清理非出圖語言：移除 "ChatGPT"、"全角色卡品質補強" 等系統控制標籤

**缺點：**
- ⚠️ 治標不治本：只處理道具持物，未解決服裝/光影模板污染
- ⚠️ 依賴人工清理：74 組資料需逐一檢查場景合理性
- ⚠️ 擴展性差：新主題仍可能出現類似問題（例如都市場景出現披帛）
...
#### **階段 1：立即執行報告 B（P0，本週完成）**

**理由：**
1. 解決緊急矛盾：ChatGPT 同時看到「持酒盞」和「不預設拿酒杯」導致輸出不穩定
2. 清理流程文字：移除 "ChatGPT"、"全角色卡品質補強" 等非出圖語言，提升咒語純度
3. 快速驗證：`npm run check` 立即看到改善效果

**執行清單：**
- [ ] 修改 `actionQualityGuardText()` 措辭（data.js 約行 7132）
- [ ] 修改 `profileFactory.js` 第 75 行道具句
- [ ] 建立 `normalizeSceneActionProps()` 函數
- [ ] 強化 `isBedchamberConsortProfile()` 正則
- [ ] 人工清理 74 組寢宮持杯動作（handcraftedConsortProfiles.js）
- [ ] 執行 `npm run check` 驗證
- [ ] 新增測試：`WORLD_LAYER_PROFILES` 輸出不得含 "ChatGPT"、"全角色卡品質補強"

---

#### **階段 2：準備報告 A 模組系統（P1，下週評估）**

**前置作業：**
1. 讀取 `doc/核心咒語規範.txt` 了解當前結構
2. 設計向後相容遷移方案：
   - 現有 2,202 張角色卡如何對應到新模組？
   - 角色卡是否需要新增 `theme` 欄位？
   - 還是透過 `parentCategory` 推斷主題？
3. 小範圍試點：
   - 先做 10 張「巴黎咖啡館」（都市模組）測試
   - 再做 10 張「仙俠宮廷」（皇后模組）對比
   - 驗證模組切換邏輯正確性
4. 評估風險：
   - 現有測試 59/59 是否需要全部重寫？
   - `buildChatGptInstruction()` 重構影響範圍？
   - 是否需要保留舊邏輯作為 fallback？

**實作順序：**
1. 先寫模組定義文檔（YAML 格式，放入 `doc/核心咒語規範.txt`）
2. 建立 `src/modules/` 目錄結構（先不接入主邏輯）
3. 實作 `THEME_MODULES` 映射表與 `generatePrompt()` 函數（獨立測試）
4. 小範圍 A/B 測試（10 張卡用新邏輯，10 張卡用舊邏輯對比）
5. 確認無退化後，逐步遷移全部角色卡
6. 最後移除舊邏輯

**風險控制：**
- 保留舊 `buildChatGptInstruction()` 作為 `buildChatGptInstructionLegacy()`
- 新增 feature flag：`USE_MODULAR_SYSTEM = true/false`
- 測試覆蓋：新舊兩套邏輯對同一張卡輸出 diff 不超過 10%
...
| 工作 | 屬於 | 優先級 | 互斥性 |
|------|------|--------|--------|
| 清理流程文字（移除 "ChatGPT"） | 報告 B | P0 | ✅ 獨立 |
| 建立 `normalizeSceneActionProps()` | 報告 B | P0 | ✅ 獨立（模組化後仍需要） |
| 撰寫模組定義 YAML | 報告 A | P1 | ✅ 獨立（只寫文檔不改程式） |
| 人工清理 74 組持杯動作 | 報告 B | P1 | ✅ 獨立 |
| 讀取 `doc/核心咒語規範.txt` | 報告 A | P1 | ✅ 獨立 |
| 設計 THEME_MODULES 映射表 | 報告 A | P1 | ⚠️ 需等清理完成，避免映射到錯誤資料 |

---

### 核心差異總結

| 維度 | 報告 A（模組化） | 報告 B（邏輯修正） |
|------|-----------------|-------------------|
| **定位** | 重構模板架構 | 修補當前矛盾 |
| **層級** | 系統架構層 | 函數邏輯層 |
| **範圍** | 服裝/動作/光影全面模組化 | 道具持物 + 流程文字清理 |
| **工期** | 3-5 天 | 1-2 天 |
| **風險** | 高（需遷移 2,202 張卡） | 低（局部修改） |
| **治本性** | ✅ 從根本消除主題污染 | ⚠️ 治標，未解決服裝/光影污染 |
| **可逆性** | 需保留舊邏輯作 fallback | 可直接回滾 |

**結論：兩者不互斥，建議先 B 後 A**  
原因：避免大重構時還帶著矛盾指令，先清理乾淨再改架構更安全。

---

## 【當前任務】咒語模板模組化改造（2026-06-13）
...
1. **更新核心規範檔案** `doc/核心咒語規範.txt`：新增模組定義區段、定義主題分類與模組映射表、標記核心模板與可選模組
2. **同步至 coreSpec.js**：`npm run sync:spec`
3. **修改角色卡生成邏輯**：建立 `generatePrompt(roleCard, theme)` 函數，依主題動態組裝核心模板 + 對應模組
4. **建立模組庫檔案**：`src/modules/` 目錄（core.js, costume.js, action.js, light.js, themeMapping.js）
5. **更新測試**：測試主題分類正確性、模組切換不出錯、不相容元素不會混入
6. **驗證**：`npm run check`

### 預期成果

**Before（當前問題）**  
巴黎咖啡館主題仍包含：透明外紗、披帛、大型外袍、戰袍、dominant cinematic silhouette、高亮商業古裝電影海報

**After（改造後）**  
巴黎咖啡館主題自動使用：核心模板 + 都市模組（風衣、西裝外套）+ 都市動作（端咖啡、翻書、倚窗凝視）+ 都市攝影（自然窗光、街燈、咖啡館暖光）+ Editorial Fashion Photography 風格
...
- 圖片批次分析新增角色卡時，`npm run check` 通過只代表格式、測試、建置與 UI 驗證過關，不等於內容品質已完全可靠。
- 若任務宣稱「逐張分析大量圖片」，仍需人工抽查新增角色卡是否真的反映圖片差異，而不是只產生泛用的古裝、仙俠、暗黑、都市等模板。
- 新增角色卡後必須檢查：身份、場景、服裝、道具、色彩、光影是否有明確差異，避免只是更換名稱與顏色。

### 姿勢與道具策略（強烈推薦：讓 ChatGPT 發揮但給予邊界）

#### 核心理念：開放式引導 > 固定寫死

**不推薦：** 每張角色卡都寫死「一手持酒杯、一手扶腰、微側身 45 度」
- 問題 1：93 張角色卡都用相同模板 → 重複、無聊、失去變化
- 問題 2：場景不合理（寢宮拿酒杯、戰場拿茶盞）
- 問題 3：限制創意，無法根據情節和構圖調整

**推薦：** 讓 ChatGPT 依場景、身份、情節與構圖自然發揮，但提供姿態邏輯邊界

#### 推薦描述模板

```text
姿態依場景支撐點、角色身份、情節與構圖自然成立；
需有明確支撐點或動作理由，避免呆站與證件照；
道具僅在主題明確需要時由人物拿在手上，
也可作為陳設、光源、前景、背景或支撐點；
手、道具、布料、髮絲與特效不得遮擋五官，
重心與布料受力符合真實成年人體結構。
...
1. **避免重複感**：ChatGPT 可以根據每次主題創造不同姿態
2. **情境合理性**：道具使用符合場景邏輯
3. **更多驚喜**：每次出圖都有新鮮感和變化
4. **保持品質**：邊界規則確保不會滑向呆站或遮臉

#### 實際效果對比

**❌ 固定寫死（不推薦）：**
```
動作：一手持酒杯，一手扶腰，微微側身看向鏡頭
```
→ 所有角色卡都一樣、寢宮拿酒杯不合理

**✅ 開放式引導（推薦）：**
```
動作：姿態依寢宮支撐點自然成立，可倚靠床榻、窗台、簾幕；
道具如酒杯、燭台可作陳設或光源，不預設手持；
避免呆站，臉部朝向鏡頭，手不遮臉
```
→ ChatGPT 會設計：扶床榻、倚枕、撩簾、坐榻沿等多樣化姿態

### 服裝 Layer 策略（強烈推薦：保持模板系統，不要完全開放）

#### 核心理念：結構化模板 > 完全自由發揮

**不推薦：** 完全交給 ChatGPT 自由發揮服裝設計
- 風險 1：容易變成「AI 仙女裝」（半透明、漂浮、不可穿戴）
- 風險 2：失去文化考據（亂混朝代元素）
- 風險 3：容易滑向低俗（露肩、透視、緊身）
- 風險 4：93 張角色卡品質不一致

**推薦：** 保持 10 層結構化模板，使用變數注入保持靈活性

#### 10 層 Layer 系統設計邏輯

```text
Layer 1：內層襯底 - 建立真人成年骨架、肩頸與胸腔厚度
Layer 2：基礎支撐 - 貼合角色身份，保持正常軀幹深度與可穿戴邏輯
Layer 3：主體輪廓 - 形成第一視覺，不追求極端腰臀比例
Layer 4：外紗披帛 - 依場景空氣流動自然垂落
Layer 5：配飾珠寶 - 固定腰線、肩線與角色文化符號，反射真實光源
Layer 6：大型外袍 - 形成 dominant cinematic silhouette
Layer 7：身份飾件 - 建立系列辨識度，不遮擋臉部
Layer 8：道具記憶點 - 可作手持、支撐點、桌面、前景、背景，不必固定拿在手上
Layer 9：頭飾髮飾 - 貼合真人頭型與髮際線，完整露出原始五官
Layer 10：整體融合 - 色彩、服裝、道具、場景與光影共同形成電影主視覺
```

#### 為什麼需要 10 層？

1. **防止 AI 過度發揮**：
   - Layer 1-3 確保真實的身體結構和可穿戴邏輯
   - 防止透視、漂浮、違反物理的服裝

2. **確保文化符號**：
   - Layer 5-7 確保階級、地域、文化辨識度
   - 防止亂混朝代元素
...
雖然保持 10 層結構，但 ChatGPT 仍有大量發揮空間：
...
```javascript
Layer 1: `${palette}內層真絲或細織襯底建立真人成年骨架、肩頸與胸腔厚度`
Layer 2: `${costumeCore}基礎支撐結構貼合角色身份，保持正常軀幹深度與可穿戴邏輯`
Layer 8: `${prop}作為角色記憶點，可依場景作為手持、支撐點、桌面、床邊、前景、背景或近景道具，不必固定拿在手上`
```

→ 每張角色卡通過變數（palette、costumeCore、prop）注入獨特性
...
```text
L1 內層襯底：建立真人身體厚度、肩頸與軀幹可穿戴基礎
L2 主體服裝：形成角色身份、文化屬性與第一視覺輪廓
L3 外層披帛或外袍：增加空氣感、動態與電影級剪影
L4 配飾珠寶：強化階級、地域、儀式感與系列辨識度
L5 道具或場景連結：道具可手持、支撐、前景陳設或背景符號，依場景合理使用
L6 光影材質：讓布料、金屬、珠寶、霧氣與膚色有真實攝影分離
L7 整體融合：統一色彩、姿態、道具、場景與光線，避免各元素互相搶戲
```

### 新增角色卡審核清單

- [ ] `id` 是否符合既有命名規則，避免缺少 `style-ref-` 等必要前綴
- [ ] `category` / `parentCategory` 是否能被 UI 分類正確路由，不能產生 ghost category
- [ ] 是否補齊 `cupSize`、`costume`、`makeup`、`scene`、`sceneEnvironment`、`sceneAction`、`sceneLighting`、`layers` 等必要欄位
- [ ] 姿勢描述是否給邊界但不寫死（開放式引導）
- [ ] 道具是否符合場景，不應每張都手持固定物件
- [ ] 服裝 Layer 是否服務真人可穿戴與角色辨識，而不是單純堆疊華麗詞
- [ ] 每張角色卡是否有明確的視覺差異（不只是改名字和顏色）
- [ ] 新增後必跑：
...
本次從風格範例資料夾讀取 59 張圖片，新增 22 張角色卡：
- 15 張加入 `styleReferenceProfiles.js`（風格範例轉卡系列）
- 7 張加入 `styleAnalysisProfiles.js`（風格分析系列）

修復問題：
- 補齊所有角色卡的 `cupSize`、`category`、`layers` 等必要欄位
- 修正 category 分類對應，避免 ghost category
- 移除未使用的「風格範例精選」分類
...
- ✅ lint 0 錯誤
- ✅ build 成功
- ✅ UI 驗證通過（desktop & mobile）

確認：目前的「開放式姿態 + 結構化 Layer」策略有效且可擴展。

---

### 2026-06-12 全角色卡動態分鏡規則

所有角色卡的 `sceneAction` 都必須使用「電影分鏡動作公式」，不是單純站姿或擺拍：

```text
一個主動作 + 一個動態核心 + 一個支撐點 + 一個道具位置 + 一個動態來源 + 一個前景互動 + 臉部保護
```

#### 必須包含的設計點

- **一個主動作**：轉身、停步、起身、掀簾、按弦、扶欄、踏階、回望、整冠、收袖、俯身、側坐等，只選一個核心，不要堆太多動作。
- **一個動態核心**：剛轉身、剛起身、剛停步、剛按弦、剛掀簾、剛踏階、剛回望等，讓畫面像被電影鏡頭捕捉的一瞬間。
- **一個支撐點**：欄杆、桌沿、階梯、窗台、王座、樂案、鏡台、石柱、床榻、帷幕、花枝或其他場景物件，避免無支撐呆站。
- **一個道具位置**：道具可手持、陳設、光源、前景、背景或身側記憶點；只有主題明確需要時才由人物拿在手上。
- **一個動態來源**：布料、髮絲、珠簾、花枝、燈火、水面反光、霧氣或披帛需跟隨主動作形成方向性。
- **一個前景互動**：宮燈、珠簾、花枝、銅鏡、樂器、玉璽、欄杆、水面或階梯邊緣可用來壓鏡、遮擋與引導視線。
- **臉部保護**：手、道具、披帛、髮絲與特效不得遮擋五官，臉部需正面或微側正面清楚可辨識。

#### 寫法範例

```text
人物剛從玉璽漆案旁轉身抬眼，一手扶住案邊穩住重心，另一手收住廣袖下緣；玉璽置於前景案面作權力記憶點，鳳冠珠旒與廣袖因轉身產生輕微延遲擺動，青銅宮燈與香煙形成前景壓鏡，臉部微側正面清楚看向鏡頭。
```

#### 禁止方向

- 不要只寫「站於中軸」「正面凝視」「雙手自然垂放」。
- 不要一張卡同時塞太多動作。
- 不要把所有道具都變成手持。
- 不要用同一套動作模板批量改角色卡。
...
---
## 2026-06-13 XXXXXX.txt 分析補充：公式感來源與落地修正方向

### 讀檔結論
`XXXXXX.txt` 的重點不是「字數多寡」，而是它把一個很重要的分界講清楚了：  
**真正有效的 prompt，不是把所有好詞全塞進去，而是把「通用骨架」和「題材專用詞」拆開。**

### 這份文件指出的有效內容
以下內容應視為可跨題材保留的高價值核心，而不是每次都重複堆疊的裝飾詞：
...
### 這份文件指出的風險
目前專案容易出現的公式感，不是來自單一詞，而是來自一整組「跨題材硬套」的視覺詞：
...
這些詞在神話、仙俠、宮廷、黑暗華麗題材很合理，但如果直接套到旅遊、城市、咖啡館、展覽、日常人物，就會讓 prompt 看起來像同一套模板反覆套版。

### 對目前專案的直接判斷
目前 Layer 系統的問題，不是 Layer 數量太多，而是：

1. **固定選層太多**
2. **每層句型太一致**
3. **題材不敏感的視覺詞被大量共用**

這會讓輸出變成「格式穩定，但畫面語氣相同」，最後看起來就會很公式化。

### 建議落地策略
#### P0：保留主 prompt 骨架，不要把核心控制詞刪掉
應保留的骨架是：
...
這些是穩定的控制層，不應因為要縮字就整段刪掉。

#### P0：把題材敏感詞移出通用區
以下詞要改成依題材選配，不要再當成固定增強詞：
...
#### P1：建立更細的 `highlightLayers` 預設策略
不同題材應該有不同預設層，例如：

- 神話 / 古風：保留飄逸、紗感、神性光影
- 現代 / 城市 / 旅拍：保留服裝剪裁、色塊、姿勢、鏡頭感
- 宮廷 / 黑暗華麗：保留材質、層次、金屬感、儀式性

也就是說，`highlightLayers` 不應只做「挑幾層」，還要做「依題材挑對層」。

#### P1：新增「純咒語模式」
應再拆一層輸出模式，讓結果更適合直接貼到不同生圖模型：
...
這樣才能讓 prompt 保持高辨識度，同時不會越來越像公式模板。
---
## 2026-06-13 AI 方案整合結論：Codex 與 Kiro 的共同執行版

### 統一判斷
兩份 AI 的分析不是互斥，而是互補。

- **Codex** 先抓住整體方向：先做 prompt 去重與層級化，再處理姿態與道具規則，最後才做大型資料模組化。
- **Kiro** 再把方向拆成可執行順序：先修最傷品質的固定 bypass，再加 `highlightLayers`，再做 `enrichSceneAction` 去重，最後才移固定長句與整理資料結構。

### 合併後的結論
如果要把兩份方案合成一個真正可落地的版本，應該採用這個順序：

1. **先拆 `buildFinalCostumeText()` 的固定 bypass**
   - 這是最傷的公式來源。
   - 影響所有暗黑 / 魅魔 / 墮天使系列。
   - 不先修這個，後面的層級調整效果會被蓋掉。

2. **再加 `highlightLayers` 的條件式選取**
   - 讓不同題材、不同角色卡挑不同層。
   - 先打破「所有卡都輸出同一組層」的問題。

3. **接著做 `enrichSceneAction()` 的語義去重**
   - 讓原始 sceneAction 與補強守則不再互相重複或打架。
   - 這一步會直接改善姿態與道具的衝突感。

4. **再把固定長句移出主 prompt**
   - 像 `realismQuality`、`directorLens` 這類穩定規則，應該進核心規範層。
   - 主 prompt 只留差異化內容。

5. **最後才做大型資料模組化**
   - 包含 profile 分檔、手工清單清理、bundle 拆分。
   - 這些重要，但應該排在內容品質穩定之後。

### 這個合併版的重點
- **Codex 負責方向**：去重、分層、姿態與道具的規則化。
- **Kiro 負責順序**：先修最傷品質的固定模板，再做層級與資料結構。
- **XXXXXX.txt 負責原則**：通用骨架保留，題材詞要模組化，避免跨題材模板化。

### 對專案的實際意義
這代表接下來的改動不該是「一直加詞」，而應該是：
...
### 最終結論
**不要二選一，直接採用「Codex 的方向 + Kiro 的順序」的合併版。**
這樣才能同時保留結構、降低公式感、又不會讓改動風險一下子太大。
---
## 2026-06-13 最佳化實作完成：題材化 fabric / highlightLayers 收斂

### 已完成
- `src/promptEngine.js` 已補回並整理 `inferCategoryVisualModeText()`、`inferCategoryColorText()`、`inferCategoryMaterialText()`、`inferCategoryFabricText()`。
- `resolveCostumeLayerIndices()` 已調整為更題材化的預設選層，旅拍 / 都市 / 藝廊 / 咖啡館類不再預設拉出過重的外袍模板。
- `buildFinalStyleText()` 已改成可以根據主題推導更合適的布料動態，避免所有主題都走同一套長飄紗語法。
- `scripts/verify-ui.mjs` 已同步更新為新規則，暗黑主題驗證改看暗黑語氣的布料動態，而不是硬性要求東方飄紗模板。

### 驗證結果
- `npm.cmd run lint`：通過
- `npm.cmd run test`：通過，67 / 67
- `npm.cmd run check`：通過
- desktop / mobile verify：通過，無 console error、無水平溢出

### 本次實作的重點
- 保留暗黑 / 宮廷 / 神話的高戲劇性，但不把東方飄紗模板錯套到不合適的主題。
- 讓旅拍、都市、藝廊、咖啡館等題材的預設 Layer 更收斂，避免外袍、披風、超長飄紗一路重複。
- 保持 prompt 骨架穩定，同時把題材詞和布料動態做成可分流的內容層。
